...
ggplot(rdat, aes(x = predprob)) + geom_histogram(bins=100) + 
  theme_bw() + geom_vline(xintercept = 0, linetype = 2, color = "red")
...
x1 <- c(1,as.numeric(logitdata[1,-1]))
x1 #first observation (plus intercept)
logit_beta <- logit1$coefficients
logit_beta #estimated coefficients
logit_beta %*% x1
